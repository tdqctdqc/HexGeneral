using System.Collections.Generic;
using GodotUtilities.GameData;
using GodotUtilities.Logic;
using HexGeneral.Data.Components;
using HexGeneral.Game;
using HexGeneral.Game.Components;

namespace HexGeneral.Logic.Procedures;

public class EmbarkProcedure : GodotUtilities.Server.Procedure
{
    public ERef<Unit> Unit { get; private set; }
    public ModelIdRef<Mobilizer> Mobilizer { get; private set; }
    public HexRef IntoHex { get; private set; }

    public EmbarkProcedure(ERef<Unit> unit, ModelIdRef<Mobilizer> mobilizer, HexRef intoHex)
    {
        Unit = unit;
        Mobilizer = mobilizer;
        IntoHex = intoHex;
    }

    public override void Handle(ProcedureKey key)
    {
        var unit = Unit.Get(key.Data);
        var from = unit.GetHex(key.Data.Data());
        key.Data.Data().MapUnitHolder.SetUnitPosition(unit,
            IntoHex, key);
        new AddEntityComponentProcedure<Unit>(
            unit.MakeRef(),
            new EmbarkedComponent(unit.MakeRef(),
                unit.Components.Get<IMoveComponent>(key.Data),
                Mobilizer)).Handle(key);
        unit.Components.Get<MoveCountComponent>(key.Data)
            .SpendMove(1f, key);
        key.Data.Data().Notices.UnitMoved?.Invoke((unit, new List<HexRef>{from.MakeRef(), IntoHex}));
    }
}